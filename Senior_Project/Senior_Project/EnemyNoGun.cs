...
namespace Senior_Project
{
    public class EnemyNoGun : Enemy
    {
        ...
        /// <author>Douglas Wardle</author>
        /// <date>10/4/2017</date>
        public void Update(GameTime a_GameTime, Player a_MainPlayer, List<Enemy> a_RoomEnemies, int a_RandMoveCount, int a_RandMoveDelay)
        {
            if(this.m_IsActive)
            {
                if (this.m_MoveCount > 0)
                {
                    this.MoveToPlayer(a_MainPlayer, a_RoomEnemies);
                    this.m_MoveCount--;
                }
